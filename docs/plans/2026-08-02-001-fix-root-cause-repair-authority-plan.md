...
8. **Bind advancement to an observed worktree delta.** After stage work, inventory emits an exact content-blind delta digest. Advance recaptures and requires that digest before replacing the checkpoint. Rejected alternative: a boolean worktree bypass. Reason: a blanket bypass can authorize an extra mutation that arrived after the stage inventory.
9. **State the custody boundary honestly.** Target-writing workflow agents are part of the trusted computing base. Self-digests and checkpoint chains detect accidental or uncoordinated drift but do not claim authentication against a hostile same-user process. Rejected alternative: describe a self-hash as a MAC. Reason: both the sidecar and Markdown evidence are writable under the same yolo authority.
...
The candidate repository-state tool owns deterministic capture, comparison, inventory, and checkpoint advancement. Stage instructions consume its structured verdict rather than equating fingerprint equality with authority equality. The task-local `repository-authority.json` moves with the task folder between stages, lives under excluded `.hive-state`, is content-blind, is atomically replaced, is bound into stage evidence, and is validated before comparison.
