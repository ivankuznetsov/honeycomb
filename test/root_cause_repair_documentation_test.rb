...
class RootCauseRepairDocumentationTest < Minitest::Test
  ...
  def test_root_readme_discloses_package_risk_and_publication_boundary
    readme = File.read(File.join(ROOT, "README.md"))

    assert_includes readme, "packages/root-cause-repair/1.0.0"
    %w[immutable package Git-only high-risk uncommitted].each do |term|
    ...
    assert_match(/package presence alone[\s\S]*public installation/i, readme)
  end
