...
research runs first, can read the project repository, and cannot use the
network. Web research can read only `brief.md` through Claude's file-qualified
scope and can search and fetch public pages; portable runners that cannot
enforce that scope are rejected. The raw `.txt` evidence files are not
automatically embedded in later prompts: synthesis reads them deliberately and
emits the decision-focused `research.md`. No stage has shell, secret, or
