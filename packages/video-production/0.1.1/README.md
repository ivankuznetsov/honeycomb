...
Workflow stages use the stage-specific wrappers to preserve approval and
evidence validation. The shared implementation is intentionally non-executable.
Actors have normal tools; the detached owner receipt still requires the
owner's private key, which must remain outside the agent's access.
...
ruby -e 'path, revision = ARGV; bytes = File.binread(path); raise "unexpected seed" unless bytes.scan("SOURCE_COMMIT_REQUIRED").length == 2; File.binwrite(path, bytes.gsub("SOURCE_COMMIT_REQUIRED", revision))' packages/video-production/0.1.1/manifest.yml "$source_revision"
ruby script/honeycomb-manifest packages/video-production/0.1.1
ruby script/honeycomb-manifest --check packages/video-production/0.1.1
ruby script/honeycomb-validate packages/video-production/0.1.1
```
