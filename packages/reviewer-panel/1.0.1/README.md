...
Map Reviewer Panel slots to the execution profiles appropriate for each role.
The workflow does not require provider-specific tool-scope enforcement.
...
Correctness, security, and reliability remain analytical review roles, while
the repair actor applies changes. All actors use normal tools. The default
path needs no network or secret access; avoid concurrent repository writers.
...
This `1.0.1` directory is an immutable package source. Its canonical manifest
