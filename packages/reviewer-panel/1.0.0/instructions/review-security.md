# Review the state-bound change for security
...
review beyond the declared change and minimum adjacent trust boundaries.

Review only the security lens: input and output trust boundaries, permissions,
authentication and authorization, secret handling, injection, unsafe parsing,
path and command construction, data exposure, and fail-open behavior introduced
or exposed by the change. Cover every materially relevant changed surface or
state why this lens does not apply. Do not turn speculative hardening or style
preferences into blocking findings.
